...
import { Dimensions, Platform, ScrollView, TouchableOpacity, View, TouchableWithoutFeedback, Linking } from 'react-native';
...
type ScreenRouteProp = RouteProp<ScreensParamList, 'HomeworkStudentCom'>;

type Props = {
  route: ScreenRouteProp;
};

export const HomeworkStudentCom: React.FC<NavigatorComponentProps & Props> = observer(
  ({ navigation, route }): JSX.Element => {
    ...
    useEffect(() => {
      if (route.params?.lectureId) {
        homeworkInterStore.getHomeworkInfoDetail({ infoId: homeworkInterStore.homeworkInfoDetail.id }).then(() => {
          if (homeworkInterStore.homeworkInfoDetail.content) {
            setContent(homeworkInterStore.homeworkInfoDetail.content);
          }
        });
      }
      return () => {
        homeworkInterStore
          .getHomeworkInfoDetail({
            infoId: homeworkInterStore.homeworkInfoDetail.id
          })
          .then((res) => {
    ...
    useEffect(() => {
      ...
      if (homeworkCreateStore.isUploadCom) {
        baseView.current.showMessage({ text: '提交作业成功', delay: 3 });
    ...
    const updateHomework = async () => {
      ...
      if (userStore.userInfoDetail?.id !== undefined && userStore.userInfoDetail.business?.id !== undefined) {
        if (content && content.length <= 0) {
          ...
          baseView.current.showLoading({ text: '正在上传', delay: 30 });
          homeworkCreateStore.allFileList = fileList;
          homeworkCreateStore.isDoing = 2;
          let fileIds: string[] = [];
          homeworkInterStore.homeworkInfoDetail.homeworkFilesSubmit?.map((item) => {
            fileIds.push(item.id);
          });
          homeworkCreateStore.fileIds = fileIds;
          console.log(fileIds, homeworkCreateStore.allFileList, '作业文件');
          if (homeworkCreateStore.allFileList.length > 0) {
            console.log(homeworkCreateStore.fileIds, homeworkCreateStore.allFileList, '这些资源被上传了1');
            if (homeworkInterStore.homeworkInfo.homeworkLessonStudentList?.filter((item) => item.student?.id === userStore.userInfoDetail.id).length) {
              const res = await homeworkCreateStore.uploadForAliOSS({
                lessonType: homeworkInterStore.homeworkInfo.lessonType,
                content: content,
                lectureId: homeworkInterStore.homeworkInfo.homeworkLessonStudentList?.filter((item) => item.student?.id === userStore.userInfoDetail.id)[0].id,
                userId: userStore.userInfoDetail.id,
                businessId: userStore.userInfoDetail.business?.id
              });
              console.log(res, '按钮回调');
            } else {
              baseView.current.showMessage({ text: '您没有这节作业', delay: 5 });
            }
          } else {
            console.log('没有新资源需要上传');
            console.log(homeworkCreateStore.fileIds, '这些资源被上传了2');
            homeworkCreateStore
              .submitHomework({
                lessonType: homeworkInterStore.homeworkInfo.lessonType,
                content: content,
      ...
      return (
        ...
        >
          ...
          <Icon
            ...
            onPress={() => {
              homeworkInterStore.homeworkInfoDetail.homeworkFilesSubmit = homeworkInterStore.homeworkInfoDetail.homeworkFilesSubmit?.filter(
      ...
      return (
        <View style={[]}>
          <View style={[tw.mY3, { backgroundColor: colors.background }]}>
            <TouchableOpacity style={[tw.m4, tw.flexRow, tw.justifyBetween, tw.itemsCenter]}>
              <View>
                <Text style={{ fontSize: isAndroid ? 10.5 : 12, color: colors.text, marginBottom: isAndroid ? 0 : 3 }}>作业标题</Text>
                <Text style={[{ fontSize: 17, color: colors.text, marginTop: 1 }]}>{homeworkInterStore.homeworkInfo.title}</Text>
              </View>
            </TouchableOpacity>
          </View>
