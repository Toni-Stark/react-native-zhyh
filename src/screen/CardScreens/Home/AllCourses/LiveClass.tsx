...
import { LESSON_TYPE_GROUP } from '../../../../common/status-module';
import { AllCoursesPlaceholder } from '../../../../component/skeleton/AllCoursesPlaceholder';
import { observer } from 'mobx-react-lite';

type Props = {};
export const LiveClass: React.FC<NavigatorComponentProps & Props> = observer(
  ({ navigation }): JSX.Element => {
    ...
    useEffect(() => {
      (async () => {
        await homeStore.getAll(LESSON_TYPE_GROUP, true);
      })();
    }, [homeStore]);
    ...
    const getAddData = async () => {
      if (homeStore.allSearchLiveClass.length % 6 === 0) {
        await homeStore.getAll(LESSON_TYPE_GROUP, false);
      }
    };
    ...
    const renderContent = (): JSX.Element => {
      if (lessonDetailStore.lessonDetailPlaceholder) {
        ...
        return (
          <FlatListCard
            type={LESSON_TYPE_GROUP}
            getAddData={getAddData}
            getNewData={getNewData}
            allSearchSmallClass={homeStore.allSearchLiveClass.slice()}
