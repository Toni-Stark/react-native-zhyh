...
import { LESSON_TYPE_VOD } from '../../../../common/status-module';
import { AllCoursesPlaceholder } from '../../../../component/skeleton/AllCoursesPlaceholder';
import { observer } from 'mobx-react-lite';

type Props = {};
export const OnDemandClass: React.FC<NavigatorComponentProps & Props> = observer(
  ({ navigation }): JSX.Element => {
    ...
    useEffect(() => {
      (async () => {
        await homeStore.getAll(LESSON_TYPE_VOD, true);
      })();
    }, [homeStore]);
    ...
    const getAddData = async () => {
      if (homeStore.allSearchDeMandClass.length % 6 === 0) {
        await homeStore.getAll(LESSON_TYPE_VOD, false);
      }
    };
    ...
    const renderContent = (): JSX.Element => {
      if (lessonDetailStore.lessonDetailPlaceholder) {
        ...
        return (
          <FlatListCard
            type={LESSON_TYPE_VOD}
            getAddData={getAddData}
            getNewData={getNewData}
            allSearchSmallClass={homeStore.allSearchDeMandClass.slice()}
