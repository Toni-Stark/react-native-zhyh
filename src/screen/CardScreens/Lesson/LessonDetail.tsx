...
export const LessonDetail: React.FC<NavigatorComponentProps & Props> = observer(
  ({ navigation, route }): JSX.Element => {
    ...
    const updateSignUp = useCallback(() => {
      lessonDetailStore.getLessonLiveDetail(route.params.lessonId).then(() => {
        // console.log('进入页面');
    ...
    const getCourseList = async () => {
      console.log(55555);
      if (lessonDetailStore.lessonDetail?.id) {
        navigation.navigate('Main', {
          screen: 'LessonPlanList',
          params: { lessonId: lessonDetailStore.lessonDetail.id },
      ...
      return (
        <View style={[tw.mB2]}>
          ...
          {lessonDetailStore.lessonDetail && lessonDetailStore.lessonDetail.description && lessonDetailStore.lessonDetail.description.length > 0 ? (
            <View style={[tw.leadingLoose, tw.flex1, tw.pX3]}>
              <Text>
                {t('lessonDetail.detail')}: <Text style={[tw.fontLight, { fontSize: 12 }]}>{lessonDetailStore.lessonDetail.description}</Text>
              </Text>
            </View>
          ) : null}
        </View>
    ...
    const renderLessonHeader = () => {
      return (
        <View>
          ...
          {/*<View style={[tw.bgBlue400, tw.absolute, tw.p2, { top: 0, left: 0, borderBottomRightRadius: 8 }]}>*/}
          {/*  <Text style={[tw.textWhite, { fontWeight: 'bold' }]}>公开课</Text>*/}
          {/*</View>*/}
          <Divider />
        </View>
      );
    ...
    const courseItem = (infoDetail): JSX.Element[] => {
    ...
    const joinInLearning = async () => {
      ...
      const res = await lessonDetailStore.joinNow(lessonDetailStore.lessonDetail?.id);
    ...
    const goToLearning = () => {
      navigation.navigate('Main', { screen: 'LectureList', params: { lessonId: lessonDetailStore.lessonDetail?.id }, options: { animationEnabled: false } });
    };
