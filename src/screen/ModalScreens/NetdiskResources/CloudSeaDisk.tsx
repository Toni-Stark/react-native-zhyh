...
import { Animated, FlatList, Linking, Platform, RefreshControl, ScrollView, TextInput, TouchableHighlight, TouchableOpacity, View } from 'react-native';
...
export const CloudSeaDisk: React.FC<NavigatorComponentProps & Props> = observer(
  ({ navigation }): JSX.Element => {
      ...
      RNFS.mkdir(makeDir + '/xueyue-temp-file').then(() => {
        ...
        RNFS.copyFile(url, destPath).then(() => {
          RNFS.stat(destPath).then((fileResult) => {
            ...
            if (filePath) {
              if (userStore.userInfoDetail.business?.id && userStore.userInfoDetail.id) {
                console.log(filePath, userStore.userInfoDetail.business.id, userStore.userInfoDetail.id, name, '此时文件的参数');
    ...
    let canAdd = true;
    const loadAddData = () => {
      console.log('添加新文件', cloudSeaDiskStore.allResourcesList.length);
      if (canAdd) {
        canAdd = false;
        if (cloudSeaDiskStore.allResourcesList.length % 10 === 0) {
          console.log(cloudSeaDiskStore.allResourcesList.length, '所有文件数量');
